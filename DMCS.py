...
from lsst.ctrl.iip.const import VISIT_ID
...
class DMCS(iip_base):
    ...
    def __init__(self, filename):
        ...
        self.pub_base_broker_url = "amqp://%s:%s@%s" % \
                                   (self.service_user, self.service_passwd, self._base_broker_addr)
    ...
    def on_ocs_message(self, ch, method, properties, msg_dict):
        ...
        try:
            ...
            LOGGER.debug('Message and properties from DMCS callback message body is: %s and %s',
            ...
            msg = "DMCS received unrecognized message type: %s" % e.args
            LOGGER.error(msg)
            if self.DP:
                print(msg)
            raise L1Error("DMCS ecountering Error Code %s. %s" % (str(self.ERROR_CODE_PREFIX + 35), e.args))
        except Exception as e:
            msg = "DMCS unable to on_ocs_message: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
    ...
    def on_ack_message(self, ch, method, properties, msg_dict):
        ...
        try:
            ...
            LOGGER.debug('Message and properties from ACK callback message body is: %s and %s',
                         (str(msg_dict), properties))
            ...
            msg = "DMCS unable to on_ack_message: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
    ...
    def on_gen_message(self, ch, method, properties, msg_dict):
        try:
            ...
            LOGGER.debug('Message and properties from General callback message body are:')
            LOGGER.debug(' %s and %s' % (pformat(str(msg_dict)), pformat(properties)))

            handler = self._gen_actions.get(msg_dict[MSG_TYPE])
            if handler is None:
                raise KeyError("In on_gen_message; Received unknown MSG_TYPE: %s" % msg_dict[MSG_TYPE])
    ...
    def on_fault_message(self, ch, method, properties, msg_dict):
        try:
            ...
            LOGGER.debug('Message and properties from FAULT callback message body are:')
            LOGGER.debug("%s and %s", (pformat(str(msg_dict)), pformat(properties)))

            err_code = msg_dict['ERROR_CODE']
            desc = msg_dict['DESCRIPTION']
            msg = "DMCS received fault message with error code %s -- %s" % (err_code, desc)
            LOGGER.critical(msg)
            if self.DP:
                print(msg)
    ...
    def process_enter_control_command(self, msg):
        ...
        try:
            ...
            if transition_check is False:
                LOGGER.error("bad state transition: %s" % new_state)
    ...
    def process_reset_from_fault(self, msg):
        ...
        try:
            ...
            msg = "DMCS unable to process_reset_from_start command: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
    ...
    def process_abort_command(self, msg):
        ...
        try:
            ...
            msg = "DMCS unable to process_abort_command: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
    ...
    def process_stop_command(self, msg):
        ...
        try:
            ...
            msg = "DMCS unable to process_stop_command: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
    ...
    def process_start_integration_event(self, params):
        ...
        """
        try:
            # TODO: FIX - see temp hack below...
            # CCD List will eventually be derived from config key. For now, using a list set in
            # top of this class
            ...
            # XXX - raft_list is never used in add_job method below, and apparently this code
            # was never executed, because we've never gotten a syntax error (since until now
            # raft_list wasn't initialized.  The next line of code here was just added.
            raft_list = {}
            ...
            msg = "DMCS unable to process_start_integration_event - No redis connection: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
        except L1RabbitConnectionError as e:
            msg = "DMCS unable to process_start_integration_event - No rabbit connection: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
        except Exception as e:
            msg = "DMCS unable to process_start_integration_event: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
    ...
    def process_at_start_integration_event(self, params):
        ...
        if self.STATE_SCBD.at_device_is_enabled():
            ...
            # TODO FIX replace non-pending acks with regular ack timer
            wait_time = 5  # seconds...
            self.set_pending_nonblock_acks(acks, wait_time)
        else:
    ...
    def process_fault(self, params):
        ...
        if fault_type == 'FAULT':
            ...
            LOGGER.error("DMCS seeing FAULT state from %s device with error code: %s" % (device, error_code))
            LOGGER.error("Description string is:  %s." % params['DESCRIPTION'])
        else:
            LOGGER.critical("DMCS seeing a %s state from %s device." % (device, fault_type))  # XXX
            LOGGER.critical("error code is %s and description is %s" % (error_code, params['DESCRIPTION']))
    ...
    def send_new_session_msg(self, session_id):
        ...
        try:
            ...
            msg = "DMCS unable to send_new_seesion_msg: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
        ...
        try:
            ...
            if msg_in['MSG_TYPE'] == 'START':
                if 'CFG_KEY' in msg_in:
                    ...
                    if good_cfg:
                        self.STATE_SCBD.set_device_cfg_key(device, msg_in['CFG_KEY'])
            ...
            msg = "DMCS unable to validate_transaction - can't use cfgkey: %s" % e.args
            LOGGER.error(msg)
            raise L1Error(msg)
        ...
        try:
            ...
            if transition_is_valid:
                ...
                if current_index == new_index:
                    msg = "DMCS: Invalid Same State Transition from %s to %s" % (current_state, new_state)
                    LOGGER.error(msg)
                    print(msg)
                    response = "Invalid same state transition: " + str(current_state) + " to " + new_state
    ...
    def setup_consumer_threads(self):
        ...
        try:
            ...
            msg = "DMCS unable to launch Consumers - Thread Error: %s" % e.args
            LOGGER.error(msg)
            print(msg)
            raise L1ConsumerError("Thread problem preventing Consumer launch: %s" % e.args)
        except Exception as e:
            msg = "DMCS unable to launch Consumers: %s" % e.args
            LOGGER.error(msg)
            print(msg)
    ...
    def setup_scoreboards(self):
        ...
        try:
            ...
            self.STATE_SCBD = StateScoreboard('DMCS_STATE_SCBD', self.state_db_instance,
                                              self.ddict, self.rdict, cred, config)
        except L1RabbitConnectionError as e:
            LOGGER.error("DMCS - No Rabbit Connect: %s" % e.args)
        except L1RedisError as e:
            msg = "DMCS - No Redis connect: %s" % e.args
            LOGGER.error(msg)
            print(msg)
            sys.exit(self.ERROR_CODE_PREFIX + 12)
        except Exception as e:
            msg = "DMCS can't complete setup_scoreboards: %s" % e.args
            LOGGER.error(msg)
            print(msg)
            sys.exit(self.ERROR_CODE_PREFIX + 10)
        ...
        try:
            ...
            state = self.STATE_SCBD
            state.set_device_state("AR", "OFFLINE")

            state.set_device_state("PP", "OFFLINE")

            state.set_device_state("CU", "OFFLINE")

            state.set_device_state("AT", "OFFLINE")

            state.add_device_cfg_keys('AR', self.ar_cfg_keys)
            state.set_device_cfg_key('AR', state.get_cfg_from_cfgs('AR', 0))

            state.add_device_cfg_keys('PP', self.pp_cfg_keys)
            state.set_device_cfg_key('PP', state.get_cfg_from_cfgs('PP', 0))

            state.add_device_cfg_keys('CU', self.cu_cfg_keys)
            state.set_device_cfg_key('CU', state.get_cfg_from_cfgs('CU', 0))

            state.add_device_cfg_keys('AT', self.at_cfg_keys)
            state.set_device_cfg_key('AT', state.get_cfg_from_cfgs('AT', 0))
            ...
            # Add 10 to every sequence num in case dump.rdb freq
            # did not catch an increment
            ...
            msg = "DMCS init cannot set scoreboards: %s" % e.args
            LOGGER.error(msg)
            print(msg)
            sys.exit(self.ERROR_CODE_PREFIX + 10)
