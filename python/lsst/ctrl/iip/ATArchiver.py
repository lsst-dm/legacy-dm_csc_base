...
class ATArchiver(iip_base):
    ...
    def __init__(self, filename):
        ...
        self.base_broker_url = "amqp://%s:%s@%s" % (service_user, service_passwd, self._base_broker_addr)

        self.pub_base_broker_url = "amqp://%s:%s@%s" % (service_user, service_passwd, self._base_broker_addr)
    ...
    def do_health_check(self):
        ...
        if self.set_current_fwdr() is False:
            return False
        return True
    ...
    def process_at_start_integration(self, params):
        ...
        if self.do_health_check() is False:
            ...
            self.send_fault_state_event("5751", desc, 'FAULT', 'FORWARDER')
            return

        # Add archive check when necessary...
        if self.use_archive_ctrl is False:
            pass
    ...
    def process_at_end_readout(self, params):
        ...
        if result_set == None:  #fail, ack_bool is false, and filename_list is None
            ...
            if self.use_archive_ctrl == False:
                ...
                pass
                self.process_readout_responses(readout_ack_id,
                                               msgtype, reply_queue, image_id, job_number, result_set)
        """

    def process_readout_responses(self,
                                  readout_ack_id, msgtype, reply_queue, image_id, job_number, result_set):
        self.clear_archive_ack()
    ...
    def did_current_fwdr_send_result_set(self):
        ...
        try:
            if self._fwdr_state_dict[self._current_fwdr['FQN']]['ACK_BOOL']:
