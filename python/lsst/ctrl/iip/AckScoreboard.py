...
class AckScoreboard(Scoreboard):
    ...
    def __init__(self, db_type, db_instance, cred, cdm):
        """After connecting to the Redis database instance
           ...
           4) After a timer event elapses, the scoreboard is locked & checked to see which ACKs were received.
        """
    ...
    def get_components_for_timed_ack(self, timed_ack):
        ...
        if self.check_connection():
            ...
            if exists:
                ...
                # XXX - the return used to be before this code, which is obviously wrong
                # but I'm keeping this here until we can do a further audit of how this
    ...
    def resolve_pending_nonblock_acks(self):
        ...
        #         If more than one component, there's a problem; pending acks are all unique components & IDs
        ...
        if self._redis.exists('PENDING_ACKS'):
            ...
            for kee in keez:
                # ack is here - don't check time
                if self._redis.exists(kee):
                    # Remove so we don't check again
                    self._redis.hdel('PENDING_ACKS', kee)
                else:
                    expiry = self._redis.hget('PENDING_ACKS', kee)
                    now = datetime.datetime.now().time()
                    # if timeout is expired...else do nothing and check next time
                    if now > expiry:
                        # handled by self.check_missing_acks()
                        self._redis.lpush('MISSING_NONBLOCK_ACKS', kee)
                        self._redis.hdel('PENDING_ACKS', kee)
